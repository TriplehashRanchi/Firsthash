...
// --- AddTaskForm (for use inside the modal) ---
...
export default function ProjectTaskDashboardPage() {
    ...
    const processedTasks = useMemo(() => {
        ...
        tasks.forEach((task) => {
            ...
            if (Array.isArray(task.assignee_ids)) {
                assigneeIdArray = task.assignee_ids;
            } else if (typeof task.assignee_ids === 'string' && task.assignee_ids.trim()) {
                assigneeIdArray = task.assignee_ids.split(',').map((id) => id.trim());
            }

            const assignees = assigneeIdArray.map((id) => memberMap.get(id)).filter(Boolean);

            const assigneesWithRoles = assigneeNames.map((name) => memberMapByName.get(name)).filter(Boolean);

            taskMap.set(task.id, {
                ...task,
                assignees: assigneesWithRoles.length > 0 ? assigneesWithRoles : [{ name: 'Unassigned', firebase_uid: 'unassigned', primaryRole: '' }],
                taskTitle: task.title,
                projectName: project?.name || '—',
                deliverableName: deliverable?.title || '—',
                // assignedTo: assignedTo || 'Unassigned',
                assignees: assignees.length > 0 ? assignees : [{ name: 'Unassigned' }],
        ...
        return roots.map((task, index) => ({ ...task, s_no: index + 1 }));
...
const AdminStatusToggle = ({ task, onStatusComplete }) => {
    ...
    const newStatus = isCompleted ? 'finalize' : 'completed';
        ...
        <div className="flex items-center justify-start">
            ...
            <label htmlFor={`toggle-${task.id}`} className="flex items-center cursor-pointer ml-4">
                <div className="relative">
                    <input type="checkbox" id={`toggle-${task.id}`} className="sr-only" checked={isCompleted} onChange={() => onStatusComplete(task, newStatus)} />
                ...
                <div className="ml-2 text-xs text-gray-600 font-medium select-none">{isCompleted ? 'Completed' : 'Mark Complete'}</div>
            </label>
        </div>
    );
};
