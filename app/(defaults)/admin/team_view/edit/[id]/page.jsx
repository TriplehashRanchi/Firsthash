...
const TYPE_OPTIONS = [ { value: 0, label: 'Freelancer' }, { value: 1, label: 'In-house' }, { value: 2, label: 'Manager' } ];
...
const Toast = ({ message, type, onClose }) => {
    ...
    const typeStyles = {
        success: { bg: 'bg-emerald-500', icon: 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z' },
        error: { bg: 'bg-rose-500', icon: 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z' },
    };
    useEffect(() => {
    ...
    return (
        <div className={`fixed top-5 right-5 flex items-center p-4 rounded-lg shadow-2xl text-white z-[100] ${typeStyles[type].bg}`}>
            <Icon path={typeStyles[type].icon} className="w-6 h-6 mr-3 flex-shrink-0" />
            <span>{message}</span>
        </div>
    );
};
...
const InputField = ({ label, icon, ...props }) => (
    <div>
        ...
        <div className="relative">
            ...
            <input {...props} className={`w-full px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 ${icon ? 'pl-10' : ''}`} />
        </div>
    </div>
);
...
const SelectField = ({ label, icon, children, ...props }) => (
    <div>
        ...
        <div className="relative">
             {icon && <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">{icon}</div>}
            <select {...props} className={`w-full px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 appearance-none ${icon ? 'pl-10' : ''}`}>
                {children}
            </select>
        </div>
    </div>
...
const SaveButton = ({ isLoading, children }) => (
    <button type="submit" disabled={isLoading} className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 disabled:bg-blue-400/80 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-200 transform hover:scale-105 active:scale-100">
...
const RolesModal = ({ roles, selectedRoles, setSelectedRoles, onClose }) => {
    ...
    const filteredRoles = useMemo(() =>
        roles.filter(role => role.type_name.toLowerCase().includes(searchTerm.toLowerCase())),
        [roles, searchTerm]
    );
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-95" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2 mt-4">
                    {filteredRoles.map(role => (
                        <label key={role.id} htmlFor={`role-${role.id}`} className="flex items-center p-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer transition-colors duration-150">
                            <input
                                type="checkbox"
                                id={`role-${role.id}`}
                ...
                <div className="mt-6 flex justify-end">
                    <button onClick={onClose} className="px-5 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">Done</button>
                </div>
            </div>
        </div>
    );
...
export default function MemberEditPage() {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            if (!user) { setToast({ message: "Authentication failed. Please log in.", type: "error" }); setPageLoading(false); return; }
            ...
            try {
                ...
                setForm({
                    full_name: memberData.name || '', email: memberData.email || '', phone: memberData.phone || '', alternate_phone: memberData.alternate_phone || '',
                    employee_type: memberData.employee_type, address: memberData.address || '', salary: memberData.salary || null,
                });
                let rolesToSet = [];
                if (memberData.roles && typeof memberData.roles === 'string') {
                    try {
                        rolesToSet = JSON.parse(memberData.roles).map(r => r.role_id);
                    } catch (e) { console.error("Failed to parse roles:", e); }
    ...
    const handleSubmit = async (e) => {
        ...
        const user = getAuth().currentUser;
        if (!user) { setToast({ message: "Admin not logged in.", type: "error" }); return; }
        setFormLoading(true);
        try {
            const token = await user.getIdToken();
            const payload = {
                ...form,
                salary: [1, 2].includes(form.employee_type) ? (form.salary || null) : null,
    ...
    const showSensitiveForms = [1, 2].includes(form.employee_type);

    if (pageLoading) {
        return <div className="flex justify-center items-center h-screen bg-slate-50 dark:bg-slate-900"><div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>;
    }
            ...
            <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-700 px-6 py-4 sticky top-0 z-40">
                <div className="flex items-center justify-between max-w-5xl mx-auto">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">Updating profile for <span className="font-semibold text-blue-600">{form.full_name || '...'}</span></p>
                    </div>
                    <button onClick={() => router.back()} className="px-4 py-2"><MoveLeft /></button>
                </div>
            </header>
                    ...
                    <FormCard title="Personal & Employment Details" description="Manage the member's personal information, employment type, and functional roles.">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                            ...
                            <div>
                                ...
                                <button type="button" onClick={() => setIsRolesModalOpen(true)} className="w-full text-left px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 truncate transition-colors">
                                    {getRolesButtonText()}
                                </button>
                            </div>
                            {showSensitiveForms && (
