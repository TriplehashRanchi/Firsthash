...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                const [projects, salaries, members] = await Promise.all([api.get(`${API_URL}/api/projects?status=all`), api.get(`${API_URL}/api/members/salaries`), api.get(`${API_URL}/api/members`)]);
    ...
    const processSummaryData = (projects = [], salaries = []) => {
        const totalRevenue = projects.reduce((acc, p) => acc + (parseFloat(p?.packageCost ?? 0) + parseFloat(p?.additionalCost ?? 0)), 0);
        const totalExpenses = salaries.reduce((acc, s) => acc + parseFloat(s?.amount_due ?? 0), 0);
    ...
    const processRevenueChart = (projects = [], salaries = []) => {
        ...
        projects.forEach((p) => {
            ...
            if (date && !isNaN(date.getTime())) {
                ...
                incomeByMonth[month] += parseFloat(p?.packageCost ?? 0) + parseFloat(p?.additionalCost ?? 0);
            }
        });
    ...
    const processRecentActivities = (projects = [], members = []) => {
        ...
        const activities = [...projectActivities, ...memberActivities].sort((a, b) => b.time - a.time);
    ...
    const processFinancialOverview = (projects = [], salaries = []) => {
        const incomeTransactions = projects
            ...
            .map((p) => ({
                ...
                amount: parseFloat(p?.packageCost ?? 0) + parseFloat(p?.additionalCost ?? 0),
            }));
        ...
        setFinancialOverview({
            recentTransactions: [...incomeTransactions, ...expenseTransactions].slice(0, 5),
        });
    };
    ...
    const StatCard = ({ icon, value, title, colorClass }) => (
        <div className="panel  flex items-center overflow-hidden p-4">
            <div className={`grid h-14 w-14 shrink-0 place-content-center rounded-md ${colorClass}`}>{icon}</div>
        ...
        <div>
            ...
            <div className="pt-5">
                ...
                <div className="mb-6 grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                    <StatCard icon={<IndianRupee className="h-7 w-7" />} value={`₹${summaryData.revenue.toFixed(2)}`} title="Total Revenue" colorClass="bg-primary/20 text-primary" />
                    <StatCard icon={<IconTag className="h-7 w-7" />} value={`₹${summaryData.profit.toFixed(2)}`} title="Net Profit" colorClass="bg-success-light text-success" />
                    <StatCard icon={<IconShoppingCart className="h-7 w-7" />} value={summaryData.totalProjects} title="Total Projects" colorClass="bg-warning-light text-warning" />
                    <StatCard icon={<IconCreditCard className="h-7 w-7" />} value={summaryData.ongoingProjects} title="Ongoing Projects" colorClass="bg-info-light text-info" />
                </div>
                ...
                <div className="mb-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
                    <div className="panel h-full xl:col-span-2">
                        ...
                        <div className="relative">
                            <ReactApexChart series={revenueChartData.series} options={getRevenueChartOptions()} type="area" height={325} />
                        </div>
                    </div>
                    <div className="panel h-full">
                        <h5 className="mb-5 text-lg font-semibold">Projects by Status</h5>
                        <ReactApexChart series={projectsByStatusData.series} options={getProjectsByStatusOptions()} type="donut" height={460} />
                    </div>
                </div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody>
                                    {recentProjects.map((project) => (
                                        <tr key={project.id} className="group text-white-dark hover:text-black dark:hover:text-white-light/90 transition-colors">
                                            <td className="min-w-[130px] text-black dark:text-white">
                                            ...
                                            <td>₹{(parseFloat(project?.packageCost ?? 0) + parseFloat(project?.additionalCost ?? 0)).toFixed(2)}</td>

                                            <td>
                                                <span className={`badge shadow-md ${project.status === 'completed' ? 'bg-success' : 'bg-secondary'}`}>{project.status}</span>
                                            </td>

                                            <td>
                                                <button
                                                    onClick={() => handleNavigate(project.id)}
                                                    className="flex items-center gap-1 rounded-md bg-indigo-600 px-3 py-1 text-white text-sm hover:bg-indigo-700 transition"
                                                >
                        ...
                        <div className="space-y-6">
                            {financialOverview.recentTransactions.map((item, index) => (
                                <div className="flex" key={index}>
                                    ...
                                    <span className={`whitespace-pre px-1 text-base font-semibold text-${item.type === 'income' ? 'success' : 'danger'} ltr:ml-auto rtl:mr-auto`}>
