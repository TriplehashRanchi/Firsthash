...
import { Plus, Trash2, ChevronDown, ChevronRight, Loader2, User, Briefcase, ChevronUp } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className={`p-5 bg-white rounded-xl shadow-lg border border-gray-200 mb-8 space-y-4 ${isAssigning ? 'bg-indigo-50 border-indigo-200' : ''}`}>
            ...
            />
             <input
                ...
                className="w-full border-gray-300 rounded-lg px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 text-gray-600"
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div>
                    ...
                    <select value={linkedProjectId} onChange={(e) => { setLinkedProjectId(e.target.value); setLinkedDeliverableId(''); setLinkedParentTaskId(''); }} className="w-full mt-1 border-gray-300 rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    ...
                    <select value={linkedDeliverableId} onChange={(e) => { setLinkedDeliverableId(e.target.value); setLinkedProjectId(''); setLinkedParentTaskId(''); }} className="w-full mt-1 border-gray-300 rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="">-- None --</option>
                        {deliverables.map(d => <option key={d.id} value={d.id}>{d.project_name} / {d.title}</option>)}
                    </select>
                </div>
                <div>
                    <label className="text-sm font-medium text-gray-700">Link to Parent Task</label>
                    <select value={linkedParentTaskId} onChange={(e) => { setLinkedParentTaskId(e.target.value); setLinkedProjectId(''); setLinkedDeliverableId(''); }} className="w-full mt-1 border-gray-300 rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="">-- None --</option>
            ...
            <div className="flex justify-end pt-2">
                <button type="submit" disabled={isSaving} className="flex items-center justify-center w-48 px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 disabled:bg-indigo-400 transition-colors">
    ...
    return (
        <div style={{ marginLeft: `${level * 1.5}rem` }} className="my-1 border-b border-gray-100 last:border-b-0 py-2">
            <div className="flex items-center gap-3 p-2 rounded-lg">
                ...
                {formattedDueDate && (
                    <span className={`text-sm px-2 py-1 rounded-md ${isOverdue ? 'text-red-700 bg-red-100' : 'text-gray-600 bg-gray-100'}`}>
                        {formattedDueDate}
                    </span>
                )}
                    ...
                    {selectedMemberId && (
                        <div className="mt-8">
                            ...
                            <AddTaskForm
                                ...
                                parentTasks={relevantParentTasks}
...
export default function GeneralTasksPage() {
    ...
    if (loading) {
        return <div className="flex justify-center items-center h-screen"><Loader2 className="w-12 h-12 text-indigo-600 animate-spin" /></div>;
    }

    if (error) {
        return <div className="p-8 text-center"><div className="bg-red-100 text-red-700 p-4 rounded-lg">{error}</div></div>;
    }
