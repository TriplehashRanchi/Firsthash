                ...
                <div className="p-4 space-y-4">
                    ...
                    {selectedStatus === 'other' && (
                        <div>
                            ...
                            <input type="text" value={newStatusText} onChange={(e) => setNewStatusText(e.target.value)} placeholder="e.g., Awaiting client feedback" className="w-full p-2 border rounded-md text-sm" autoFocus />
                        </div>
                    )}
                </div>
                            ...
                            <tr key={task.id} className="hover:bg-gray-50">
                                <td className="px-4 py-3 align-top">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {isLongTitle && (
                                            <button onClick={() => onShowDetails(task)} className="text-blue-500 hover:text-blue-700 flex-shrink-0" title="View full details">
                                ...
                                <td className={`px-4 py-3 align-top whitespace-nowrap ${isOverdue ? 'text-rose-600 font-medium' : 'text-gray-700'}`}>
                                ...
                                <td className="px-4 py-3 align-top whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onUpdateStatusClick(task)} className="p-2 rounded-md text-gray-500 hover:bg-indigo-100 hover:text-indigo-600 transition-colors" title="Update Status">
                                            <ClipboardEdit className="w-4 h-4" />
                                        </button>
                                        {task.voice_note_url && (
                                            <button onClick={() => onPlayAudio(task)} className="p-2 rounded-md text-gray-500 hover:bg-green-100 hover:text-green-600 transition-colors" title="Play voice note">
...
{!loading && !err && (
  <div className="space-y-8">
    {Object.entries(groupedTasks).map(([groupKey, groupTasks]) => (
        (groupTasks.length > 0) && (
            <div key={groupKey}>
                <h2 className="text-base font-semibold text-gray-800 mb-3 capitalize">
                  {groupKey.replace('_', ' ')}
                </h2>
                <TaskTable
                    tasks={groupTasks}
            ...
            <table className="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr key={p.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3"><div>{formatDate(p.minDate)} — {formatDate(p.maxDate)}</div></td>
                        <td className="px-4 py-3 text-xs text-gray-700"><div>Shoots: {p.shoots ?? 0}</div><div>Deliverables: {p.deliverablesCompleted ?? 0}/{p.deliverablesTotal ?? 0}</div></td>
                        <td className="px-4 py-3"><StatusBadge status={p.status} /></td>
                        <td className="px-4 py-3"><button onClick={() => openProjectDetails(p.id)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md border hover:bg-gray-100" title="View details"><Eye className="w-4 h-4" /><span className="text-sm">View</span></button></td>
                    </tr>
                    ))}
                </tbody>
            </table>
