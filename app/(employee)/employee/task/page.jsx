...
const Drawer = ({ open, onClose, children, title }) => (
  <>
    ...
    <div className={`fixed top-0 right-0 h-full w-full sm:w-[520px] bg-white dark:bg-gray-800 dark:text-gray-200 shadow-2xl transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="flex items-center justify-between px-4 py-3 border-b"><h3 className="font-semibold text-gray-900 dark:text-gray-200">{title}</h3><button onClick={onClose} className="p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700"><X className="w-5 h-5" /></button></div>
    ...
    return (
        <div className="fixed inset-0 bg-black/50 dark:bg-gray-900 dark:bg-opacity-30 flex justify-center items-center p-4 z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
                <div className="p-4 border-b"><h3 className="font-semibold dark:text-gray-200 text-gray-800">Update Task Status</h3><p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1">Task: {task?.title}</p></div>
                <div className="p-4 space-y-4">
                    <div>
                        <label className="block text-sm dark:text-gray-300 font-medium text-gray-700 mb-1">Choose a status</label>
                        <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="w-full p-2 border dark:bg-gray-700 dark:text-gray-200 rounded-md bg-white text-sm">
                    ...
                    {selectedStatus === 'other' && (
                        <div>
                            ...
                            <input type="text" value={newStatusText} onChange={(e) => setNewStatusText(e.target.value)} placeholder="e.g., Awaiting client feedback" className="w-full p-2 border dark:bg-gray-700 dark:text-gray-200 rounded-md text-sm" autoFocus />
                        </div>
                    )}
                </div>
                <div className="p-4 bg-gray-50 dark:bg-gray-700 flex justify-end gap-3">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 disabled:bg-indigo-400">{isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save Status'}</button>
                </div>
            </div>
        </div>
    );
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-gray-900 dark:bg-opacity-30 flex justify-center items-center p-4 z-50" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-4" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={onClose} className="p-1 rounded-full dark:text-gray-500 hover:bg-gray-100"><X className="w-4 h-4" /></button>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-gray-900 dark:bg-opacity-30 flex justify-center items-center p-4 z-50" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 dark:text-gray-200 rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b flex justify-between items-center">
                    ...
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"><X className="w-4 h-4" /></button>
                </div>
                    ...
                    {tasks.map((task) => {
                        ...
                        return (
                            <tr key={task.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td className="px-4 py-3 align-top">
                                    <div className="flex items-center gap-2">
                                        <span className="font-semibold text-gray-800 dark:text-gray-200">
                                            {isLongTitle ? `${task.title.substring(0, 70)}...` : task.title}
                                        </span>
                                        {isLongTitle && (
                                ...
                                <td className="px-4 py-3 align-top whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onUpdateStatusClick(task)} className="p-2 rounded-md text-gray-500 dark:text-gray-400 hover:bg-indigo-100 hover:text-indigo-600 transition-colors" title="Update Status">
                                            <ClipboardEdit className="w-4 h-4" />
                                        </button>
                                        {task.voice_note_url && (
                                            <button onClick={() => onPlayAudio(task)} className="p-2 rounded-md text-gray-500 dark:text-gray-400 hover:bg-green-100 hover:text-green-600 transition-colors" title="Play voice note">
...
const [taskForDetails, setTaskForDetails] = useState(null);
  ...
  const fetchAll = useCallback(async () => {
    ...
    } catch (e) { console.error(e); setErr('Failed to connect to the server.'); }
  ...
  useEffect(() => {
    ...
    const unsub = auth.onAuthStateChanged((u) => {
      if (u) fetchAll();
      else { setLoading(false); setErr('Please sign in to view your work.'); }
    });
    return () => unsub();
  }, [fetchAll]);
...
const handleCloseDetails = () => {
  ...
  const handleUpdateTaskStatus = async (task, newStatus) => {
    ...
    try {
        ...
        if (response.status === 200) { toast.success('Status updated!', { id: toastId }); await fetchAll(); }
        else { toast.error(response.data?.error || 'Failed to update.', { id: toastId }); }
    } catch (e) { toast.error('An error occurred.', { id: toastId }); console.error(e); }
  };
  
  const handleOpenStatusModal = (task) => { setTaskToUpdate(task); setIsStatusModalOpen(true); };
  const handleCloseStatusModal = () => { setTaskToUpdate(null); setIsStatusModalOpen(false); };

  // ✅ NEW: Handlers for the audio player
  const handlePlayAudio = (task) => { setAudioPlayerData({ isOpen: true, url: task.voice_note_url, title: task.title }); };
  const handleCloseAudioPlayer = () => { setAudioPlayerData({ isOpen: false, url: '', title: '' }); };
...
{!loading && !err && (
  <div className="space-y-8">
    {Object.entries(groupedTasks).map(([groupKey, groupTasks]) => (
        (groupTasks.length > 0) && (
            <div key={groupKey}>
                <h2 className="text-base font-semibold text-gray-800 mb-3 dark:text-gray-200 capitalize">
                  {groupKey.replace('_', ' ')}
                </h2>
                <TaskTable
                    tasks={groupTasks}
            ...
            <table className="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr key={p.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700x">
                        <td className="px-4 py-3"><div className="font-medium text-gray-900 dark:text-gray-200">{p.name}</div></td>
                        <td className="px-4 py-3 dark:text-gray-200">{p.clientName || '—'}</td>
                        <td className="px-4 py-3 dark:text-gray-200"><div>{formatDate(p.minDate)} — {formatDate(p.maxDate)}</div></td>
                        <td className="px-4 py-3 text-xs text-gray-700 dark:text-gray-200"><div>Shoots: {p.shoots ?? 0}</div><div>Deliverables: {p.deliverablesCompleted ?? 0}/{p.deliverablesTotal ?? 0}</div></td>
                        <td className="px-4 py-3"><StatusBadge status={p.status} /></td>
                        <td className="px-4 py-3 dark:text-gray-200"><button onClick={() => openProjectDetails(p.id)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md border hover:bg-gray-100" title="View details"><Eye className="w-4 h-4" /><span className="text-sm">View</span></button></td>
                    </tr>
                    ))}
                </tbody>
            </table>
              ...
              <>
                <section className="mb-6">
                  ...
                  <div className="text-sm grid grid-cols-2 gap-2">
                    <div><span className="text-gray-500 dark:text-gray-400">Project:</span> <span className="font-medium">{details.projectName || details.name}</span></div>
                    <div><span className="text-gray-500 dark:text-gray-400">Client:</span> {details.clientName || '—'}</div>
                    <div><span className="text-gray-500 dark:text-gray-400">Status:</span> {statusLabel(details.projectStatus || details.status)}</div>
                    <div><span className="text-gray-500 dark:text-gray-400">Total:</span> ₹{Number(details.overallTotalCost ?? details.totalCost ?? 0).toLocaleString('en-IN')}</div>
                  </div>
                </section>
                ...
                {Array.isArray(details?.deliverables?.deliverableItems) && details.deliverables.deliverableItems.length > 0 && (
                  <section className="mb-6">
                    ...
                    <div className="space-y-1 text-sm">{details.deliverables.deliverableItems.map((d) => (<div key={d.id} className="flex items-center justify-between border-b py-1"><div>{d.title}</div><div className="text-xs text-gray-500 dark:text-gray-300">{statusLabel(d.status || '')}</div></div>))}</div>
                  </section>
                )}
              </>
            )}
