...
const ExpenseFormModal = ({ isOpen, onClose, onSubmit, initialData }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (formData.productName && formData.expense) onSubmit(formData);
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/70 flex items-center justify-center p-4 z-[60] backdrop-blur-sm">
            <div className="bg-white dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 rounded-xl shadow-2xl w-full max-w-md border border-slate-300 dark:border-slate-700">
                <div className="flex justify-between items-center p-6 border-b border-slate-300 dark:border-slate-700">
                    <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-pink-500 flex items-center">
                        <FilePlus2 className="mr-3 text-purple-500" />
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div>
                        <label htmlFor="productName" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">
                            Expense Item
                        </label>
                        <input
                            id="productName"
                            ...
                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                            placeholder="e.g., Flight tickets for crew"
                            required
                        />
                    </div>
                    <div>
                        <label htmlFor="category" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">
                            Category
                        </label>
                        <input
                            id="category"
                            ...
                            onChange={handleInputChange}
                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                            placeholder="e.g., Travel, Personnel, Equipment"
                        />
                    </div>
                    <div>
                        <label htmlFor="expense" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">
                            Amount (₹)
                        </label>
                        <input
                            id="expense"
                    ...
                    <div className="flex space-x-4 pt-4">
                        <button
                            type="button"
                            onClick={onClose}
                            className="flex-1 px-6 py-3 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-600/50 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className="flex-1 px-6 py-3 rounded-lg text-sm font-bold text-white bg-gradient-to-r from-purple-500 to-pink-600 hover:opacity-90 transition-opacity shadow-lg"
                        >
            ...
            <div id="section-expenses">
                <div className="md:flex justify-between items-center mb-8">
                    ...
                    <button
                        ...
                        className="mt-4 md:mt-0 flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 hover:opacity-90 hover:shadow-lg hover:shadow-purple-500/20 transform hover:-translate-y-0.5"
                    >
                ...
                <div className="space-y-3">
                    ...
                    {(expenses || []).map((expense) => (
                        <div
                            key={expense.id}
                            className="group grid grid-cols-6 gap-4 items-center bg-white dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 hover:shadow-lg hover:shadow-indigo-500/10 transform transition-all duration-300 hover:-translate-y-1"
                        >
                            {/* Item and Category */}
                            ...
                            <div className="col-span-3 sm:col-span-1 flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <button
                                    ...
                                    title="Edit"
                                    className="p-2.5 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                                >
                                    <Edit3 size={16} />
                                </button>
                                <button
                                    ...
                                    title="Delete"
                                    className="p-2.5 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-red-600 dark:hover:text-red-500 transition-colors"
                                >
                                    <Trash2 size={16} />
                                </button>
                            </div>
                ...
                {(expenses || []).length > 0 && (
                    <div className="mt-8 bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl rounded-2xl p-8 text-white">
                        <div className="flex justify-between items-center">
                            <div>
                                <h4 className="text-lg font-semibold text-slate-300 flex items-center gap-3">
                                    <PiggyBank size={24} />
                                    Total Project Cost
                                </h4>
