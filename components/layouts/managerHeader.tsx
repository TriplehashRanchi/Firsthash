...
const Avatar: React.FC<AvatarProps> = ({ name, email, photoURL, className = '' }) => {
    ...
    if (photoURL && !errored) {
        return <img className={className} src={photoURL} alt={name || email || 'User'} onError={() => setErrored(true)} />;
    }

    return (
        <div className={`${className} grid place-content-center bg-primary text-white font-bold select-none`} title={name || email || 'User'}>
...
const ManagerHeader: React.FC = () => {
    ...
    const AuthPlaceholder: React.FC = () => <div className="h-9 w-9 rounded-full bg-gray-200 animate-pulse" />;
                ...
                <div className="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                        <Link href="/" className="main-logo flex shrink-0 items-center">
                            <img className="inline w-36 ltr:-ml-1 rtl:-mr-1" src="/assets/images/logo.png" alt="logo" />
                            {/* <span className="hidden align-middle text-2xl font-semibold transition-all duration-300 ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light md:inline">IPCStudios</span> */}
                        </Link>
                        <button
                            type="button"
                    ...
                    {/* Right: links + buttons + avatar */}
                    <div className="flex items-center gap-2 sm:gap-3">
                        <Link
                            href="/manager/team-allocation"
                 ...
                 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1
                 dark:bg-gray-300 dark:hover:bg-gray-400 dark:focus:ring-indigo-400"
                        >
                                    ...
                                    <div className="dropdown flex shrink-0">
                                        <div className="flex justify-end w-full">
                                            <Dropdown
                                                ...
                                                button={
                                                    <Avatar name={displayName} email={email} photoURL={photoURL} className="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" />
                                                }
                                            >
                                                <ul className="w-[200px] !py-0 font-semibold text-dark dark:text-white-dark dark:text-white-light/90">
                                                    <li>
                                                        <div className="flex items-center px-4 py-4">
                                                            <Avatar name={displayName} email={email} photoURL={photoURL} className="h-10 w-10 rounded-md object-cover" />
                                                    ...
                                                    <li className="border-t border-white-light dark:border-white-light/10 cursor-pointer">
                                                        <Link href="/manager/profile" className="flex items-center !py-3 px-4">
