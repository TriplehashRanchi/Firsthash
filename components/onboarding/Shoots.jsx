...
import { Trash, Plus, /*Edit2, Check,*/ X, Search, Settings2, ListChecks, FolderOpen } from 'lucide-react';
    ...
    return (
        ...
        >
            ...
            <button
                ...
                className="p-1 ml-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
                title={`Delete "${data.label}" from master list`}
            >
                <Trash size={14} />
            </button>
        </div>
...
const EditableListItem = ({ item, onDelete, isSelected, onToggleSelect, itemType = "service" }) => {
    ...
    return (
        <div className={`group flex items-center justify-between p-1.5 rounded-md w-full transition-colors duration-150
            ${isSelected ? 'bg-blue-100 dark:bg-blue-800/50 hover:bg-blue-200 dark:hover:bg-blue-700/60 border border-blue-500 dark:border-blue-600' : 'bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600/50 border border-gray-300 dark:border-gray-600'}
        `}>
            <span
                onClick={onToggleSelect ? onToggleSelect : undefined}
                className={`text-sm flex-grow truncate cursor-pointer py-1 ${isSelected ? 'font-semibold text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-200'}`}
...
const ShootRow = ({
    ...
    const handleTitleSelectChange = (selectedOption) => {
        onChange('title', selectedOption ? selectedOption.value : '');
    };
    ...
    const customSelectStyles = {
        ...
        option: (base, state) => ({ // This style will be applied by react-select to the container of our custom Option
            ...
            // or ensure these align with TitleOptionWithDelete's classes.
            // For simplicity, let's assume TitleOptionWithDelete's internal classes handle hover/selected states primarily.
            // backgroundColor: 'transparent', // Or let TitleOptionWithDelete handle it.
            // color: 'inherit', // Or let TitleOptionWithDelete handle it.
            padding: 0, // Remove padding as TitleOptionWithDelete has its own
        }),
    ...
    const inputBaseStyle = "w-full p-2 rounded border placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400";
    ...
    if (selectedServicesArray.length > 0) { selectedServicesText = selectedServicesArray.length <= 3 ? selectedServicesArray.join(', ') : `${selectedServicesArray.slice(0, 3).join(', ')}, +${selectedServicesArray.length - 3} more`; }
    ...
    return (
        <div className={`grid ${isFirst ? 'grid-cols-5' : 'grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto]'} gap-x-3 gap-y-1 mb-3 items-start pt-1`}>
            <div className="flex items-center gap-2">
                ...
                {shoot.title && masterEventTitles.includes(shoot.title) && (
    ...
    return (
        <div className="mb-6 p-4 bg-white dark:bg-gray-800/50 rounded-lg shadow-md dark:shadow-gray-700/50">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200">Shoot Details</h2>
                <button onClick={addShoot} className={addShootButtonStyles} title="Add Shoot" type="button"><Plus size={18} /></button>
            </div>
            {shoots.length > 0 ? shoots.map((shootItem, idx) => (
                <ShootRow
                ...
                <div className="text-center py-10">
                    ...
                    <div className="mt-6">
                        <button onClick={addShoot} type="button" className={addShootButtonStyles + " px-4"}><Plus size={18} className="-ml-1 mr-2" />Add New Shoot</button>
                    </div>
                </div>
            )}
        </div>
