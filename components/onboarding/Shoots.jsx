...
const TitleOptionWithDelete = (props) => {
    ...
    return (
        <div {...innerProps} className={`flex items-center justify-between group px-3 py-2 ${props.isFocused ? 'bg-gray-100 dark:bg-gray-700' : ''} ${props.isSelected ? 'bg-blue-500 text-white dark:bg-blue-600' : 'dark:text-gray-200'}`}>
            ...
            <button type="button" onClick={handleDelete} className="p-1 ml-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 opacity-0 group-hover:opacity-100" title={`Delete "${data.label}"`}>
                <Trash size={14} />
            </button>
        </div>
    );
...
const EditableListItem = ({ role, onDelete, isSelected, onToggleSelect, itemType = "role" }) => {
    ...
    return (
        <div className={`group flex items-center justify-between p-1.5 rounded-md w-full transition-colors duration-150 ${isSelected ? 'bg-blue-100 dark:bg-blue-800/50 hover:bg-blue-200 border border-blue-500' : 'bg-white dark:bg-gray-700 hover:bg-gray-50 border border-gray-300'}`}>
            <span onClick={onToggleSelect ? onToggleSelect : undefined} className={`text-sm flex-grow truncate cursor-pointer py-1 ${isSelected ? 'font-semibold text-blue-700' : 'text-gray-700'}`} title={`Click to ${isSelected ? 'deselect' : 'select'} "${role.type_name}"`}>
                <span className="mr-2 inline-flex items-center">
                    {isSelected ? <ListChecks size={16} className="text-blue-600" /> : <Plus size={16} className="text-gray-400" />}
                </span>
                {role.type_name}
            </span>
...
// --- ShootRow Component ---
const ShootRow = ({
    shoot, onChange, onRoleChange, onRoleCountChange,
    showRoleOptions, setShowRoleOptions, onDelete, canDelete, isFirst,
    masterEventTitles, onAddMasterTitle, onDeleteMasterTitle,
    masterRoles, onAddMasterRole, onDeleteMasterRole,
}) => {
    ...
    useClickOutside(rolesPanelRef, () => { if(showRoleOptions) setShowRoleOptions(); }, rolesTriggerRef);
    ...
    const customSelectStyles = { 
        ...
        menu: (base) => ({ ...base, backgroundColor: isDarkMode ? 'rgb(31, 41, 55)' : 'rgb(255, 255, 255)', borderColor: isDarkMode ? 'rgb(55, 65, 81)' : 'rgb(229, 231, 235)', borderWidth: '1px', borderRadius: '0.375rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05)', zIndex: 5000, marginTop: '4px' }),
    ...
    if (selectedRoleIds.length > 0) {
        ...
        selectedRolesText = selectedRoleNames.length <= 2 
            ? selectedRoleNames.join(', ') 
            : `${selectedRoleNames.slice(0, 2).join(', ')}, +${selectedRoleNames.length - 2}`;
    }
        ...
        <div className={`grid ${isFirst ? 'grid-cols-1 sm:grid-cols-5' : 'grid-cols-1 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto]'} gap-x-3 gap-y-2 sm:gap-y-1 mb-3 items-start sm:items-center pt-1`}>
            <div className="flex items-center gap-2 sm:col-span-1">
                ...
                {shoot.title && masterEventTitles.includes(shoot.title) && !isFirst && (
                    <button type="button" onClick={handleDeleteCurrentMasterTitle} className="p-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 disabled:opacity-50 flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-md border border-transparent hover:bg-red-500/10" title={`Delete "${shoot.title}"`} disabled={!shoot.title}>
                        <Trash size={16} />
                    </button>
                )}
            </div>
            ...
            <div className="relative">
                <input ref={rolesTriggerRef} placeholder="Roles*" readOnly onClick={() => setShowRoleOptions()} value={selectedRolesText} className={`${fullInputStyle} cursor-pointer truncate pr-10`} title={Object.keys(shoot.selectedRoles || {}).map(id => masterRoles.find(r => r.id.toString() === id)?.type_name).join(', ')} />
            ...
            {showRoleOptions && (
                <>
                    ...
                    <div ref={rolesPanelRef} className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[5000] p-4 md:p-6 rounded-lg shadow-2xl w-[90vw] max-w-2xl bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 max-h-[85vh] flex flex-col">
                        <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                            ...
                            <button onClick={() => setShowRoleOptions()} className="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"><X size={20} /></button>
                        </div>
                        <div className="flex-grow overflow-y-auto pr-1 space-y-6 custom-scrollbar-thin">
                            <section>
                                ...
                                <div className="flex gap-2 mb-4 p-3 bg-white dark:bg-gray-700/50 rounded-md border border-gray-200 dark:border-gray-600">
                                    <input type="text" placeholder="Add new role to master list" value={newRoleNameMaster} onChange={(e) => setNewRoleNameMaster(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddNewMasterRole()} className="flex-grow p-2 text-sm border rounded bg-white dark:bg-gray-600 text-gray-800 dark:text-gray-100 border-gray-300 dark:border-gray-500 focus:ring-blue-500" />
                                    <button type="button" onClick={handleAddNewMasterRole} className="px-4 py-2 text-sm rounded bg-green-600 hover:bg-green-700 text-white flex items-center gap-1.5" disabled={!newRoleNameMaster.trim()}><Plus size={16} /> Add</button>
                                </div>
                                ...
                                <section className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                    ...
                                    <div className="space-y-2.5 max-h-[calc(85vh_-_500px)] overflow-y-auto p-1 custom-scrollbar-thin">
                                        {selectedRoleIds.map(roleId => {
                                            ...
                                            if (!role) return null;
                                            return (
                                                <div key={roleId} className="flex items-center justify-between text-sm p-2.5 bg-white dark:bg-gray-700/50 rounded-md border border-gray-200 dark:border-gray-600">
                                                    <span className="text-gray-800 dark:text-gray-200 truncate pr-2" title={role.type_name}>{role.type_name}</span>
                                                    <div className="flex items-center gap-2">
                                                        <label htmlFor={`count-${shoot.id}-${roleId}`} className="text-xs text-gray-600 dark:text-gray-400">Count:</label>
                                                        <input id={`count-${shoot.id}-${roleId}`} type="number" min="1" value={(shoot.selectedRoles || {})[roleId] || 1} onChange={e => onRoleCountChange(roleId, e.target.value)} className="p-1 w-16 rounded text-sm text-center border bg-white text-gray-800 border-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" />
                                                    </div>
                                                </div>
                                            );
                                        })}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 max-h-[calc(85vh_-_400px)] overflow-y-auto p-1 custom-scrollbar-thin">
                                    {masterRoles.length > 0 ? masterRoles.map(role => (
                                        <EditableListItem 
                                            key={role.id} 
                                            role={role} 
                        ...
                        <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
                            <button type="button" onClick={() => setShowRoleOptions()} className="w-full text-white py-2.5 rounded text-sm font-semibold bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600">Done</button>
                        </div>
                    </div>
                </>
            )}
...
const Shoots = ({company, onValidChange, onDataChange, initialData }) => {
    ...
    const handleDeleteMasterRole = useCallback(async (roleIdToDelete) => {
        if (company?.id) {
            try {
    ...
    const handleRoleToggleForShoot = (id, roleId) => setShoots(prevShoots => prevShoots.map(s => { if (s.id !== id) return s; const updatedSelectedRoles = { ...(s.selectedRoles || {}) }; if (updatedSelectedRoles[roleId] !== undefined) delete updatedSelectedRoles[roleId]; else updatedSelectedRoles[roleId] = 1; return { ...s, selectedRoles: updatedSelectedRoles }; }));
    const handleRoleCountChangeForShoot = (id, roleId, count) => { const newCount = Math.max(1, Number(count) || 1); setShoots(prevShoots => prevShoots.map(s => s.id === id ? { ...s, selectedRoles: { ...(s.selectedRoles || {}), [roleId]: newCount } } : s)); };
    ...
    const addShoot = () => setShoots(prevShoots => [...prevShoots, { id: Date.now() + prevShoots.length + Math.random(), title: '', date: '', time: '', city: '', selectedRoles: {}, showRoleOptions: false }]);
        ...
        if (typeof onValidChange === 'function') {
            ...
            if (shoots.length === 1 && shoots[0].title.trim() === '' && shoots[0].date.trim() === '' && shoots[0].city.trim() === '' && Object.keys(shoots[0].selectedRoles).length === 0) {
                ...
                allShootsAreValid = shoots.every(s => 
                    s && s.title?.trim() !== '' && 
                    s.date?.trim() !== '' && 
                    s.selectedRoles && Object.keys(s.selectedRoles).length > 0
                );
            }
    ...
    useEffect(() => {
        if (typeof onDataChange === 'function') {
            const relevantShoots = shoots.filter(s => 
                ...
                s.city.trim() !== '' || Object.keys(s.selectedRoles).length > 0
            );
